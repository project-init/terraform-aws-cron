locals {
  log_group_name = "${var.service_name}-${var.environment}-${var.cron_name}-logs"
}
...
data "aws_iam_policy_document" "log-group" {
  ...
  statement {
    sid    = "${title(var.service_name)}${title(var.environment)}${title(var.cron_name)}LogGroupPolicy"
    effect = "Allow"
...
resource "aws_kms_key" "log-group" {
  description = "KMS Key for ${var.service_name} (${var.cron_name}) logs in ${var.environment}"
